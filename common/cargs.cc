...
{
  if (option->value_name != NULL) { *accessor_length += fprintf(destination, "=%s", option->value_name); }
}
...
static void cag_option_print_name(const cag_option* option, bool* first, size_t* accessor_length, FILE* destination)
{
  if (option->access_name != NULL) {
    if (*first) { *accessor_length += fprintf(destination, "--%s", option->access_name); }
    else {
...
static size_t cag_option_get_print_indention(const cag_option* options, size_t option_count)
{
  ...
  for (option_index = 0; option_index < option_count; ++option_index) {
    ...
    if (option->access_letters != NULL && *option->access_letters) {
      ...
      if (option->access_name != NULL) { indention += strlen(option->access_name) + 4; }
    }
    else if (option->access_name != NULL) {
      indention += strlen(option->access_name) + 2;
    }

    if (option->value_name != NULL) { indention += strlen(option->value_name) + 1; }

    if (indention > result) { result = indention; }
  }
...
void cag_option_print(const cag_option* options, size_t option_count, FILE* destination)
{
  ...
  for (option_index = 0; option_index < option_count; ++option_index) {
    ...
    for (i = accessor_length; i < indention; ++i) { fputs(" ", destination); }
...
static const cag_option* cag_option_find_by_name(cag_option_context* context, char* name, size_t name_size)
{
  ...
  for (i = 0; i < context->option_count; ++i) {
    ...
    if (option->access_name == NULL) { continue; }
    ...
    // names.
    if (strncmp(option->access_name, name, name_size) == 0) { return option; }
  }
...
static int cag_option_find_next(cag_option_context* context)
{
  ...
  if (context->forced_end || c == NULL) { return -1; }
...
bool cag_option_fetch(cag_option_context* context)
{
  ...
  if (new_index >= 0) { context->index = new_index; }
  else {
    return false;
  }
  ...
  if (*c == '-') {
    ...
    if (*c == '\0') { context->forced_end = true; }
    else {
